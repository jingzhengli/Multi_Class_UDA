...
class GeneralModule(nn.Module):
    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck: nn.Module, bottleneck_dim: int,
        ...
        #add
        self.moving_feature_centeriod_s = torch.zeros(self.num_classes, bottleneck_dim).cuda()  # .normal_(0, 1).cuda()
        self.moving_feature_centeriod_t = torch.zeros(self.num_classes, bottleneck_dim).cuda()  # .normal_(0, 1).cuda()
        # self.moving_feature_centeriod_t = Variable(torch.cuda.FloatTensor(self.num_classes, bottleneck_dim).fill_(0))
        # self.moving_feature_centeriod_t.requires_grad_(True)
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # q = 1.0 / (1.0 + torch.sum((features.unsqueeze(1) - self.moving_feature_centeriod_t)**2, dim=2))
        # q = q**(1+1.0)/2.0
        # q = q / torch.sum(q, dim=1, keepdim=True)
        return features, outputs, outputs_adv
    ...
    def get_parameters(self, base_lr=1.0) -> List[Dict]:
        ...
        return params
            #{"params": self.moving_feature_centeriod_t, "lr": base_lr}
