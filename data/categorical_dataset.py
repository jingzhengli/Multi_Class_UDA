...
class CategoricalSTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for d in ['source', 'target']:
            ...
            data['Label_'+d] = [self.classnames.index(self.class_set[index])] * len(data['Img1_'+d])
            
            data['Img1_'+d] = torch.stack(data['Img1_'+d], dim=0)
            data['Img2_'+d] = torch.stack(data['Img2_'+d], dim=0)
            #data['GT_'+d] = torch.stack(data['GT_'+d], dim=0)
